...
const Hero = ({ refid, isInView }) => {
  ...
  return (
    <div className="relative w-full h-auto pt-[80px] flex flex-col items-center pb-[2.5rem] lg:px-[128px] px-[16px] md:px-[64px]">
      {desktop ? (
        <>
          <h2 className="font-extrabold text-[3rem] text-center text-gray-900">
            Explore The Mastery
          </h2>
          {isInView == 1 ? (
            <div className="sticky top-[2.5rem] left-0 w-[65%] z-[110] flex items-center h-[40px] gap-10">
        ...
        <div className="w-full flex flex-col">
          ...
          <div className="w-full relative flex md:grid md:grid-cols-12 overflow-hidden">
              ...
              <div className="flex overflow-x-auto gap-2 md:gap-3 max-w-full">
                {popularSearching.mobile.map((item, idx) => (
                  <Button
                    key={idx}
                    className={
                      "text-[12px] px-[18px] rounded-full border h-[46px] border-gray-400 shrink-0"
                    }
                  >
                    {item}
                  </Button>
